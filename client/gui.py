...
class LoginWindow(QWidget):
    ...
    def handle_register(self):
        ...
        try:
            ...
            QMessageBox.information(self, "Registered", f"User '{uname}' registered with role 'user'.")
            self.users = load_users_from_excel()
...
class TeamcenterStyleGUI(QMainWindow):
    ...
    def _create_toolbar(self):
        ...
        for tid, tool in self.tools.items():
            if allowed == "all" or tid in allowed:
                tools_menu.addAction(tool["name"], lambda chk=False, t=tool: self._open_tab(t["name"], QLabel(t["description"])))
    ...
    def _open_options(self):
        QMessageBox.information(self, "Options", "User settings will be here.")
    ...
    def _show_tree_context_menu(self, pos):
        ...
        if item.parent() is None:
            menu.addAction("🔁 Refresh Project", lambda: QMessageBox.information(self, "Mock", "Project refreshed"))
            menu.addAction("➕ Add Tool", lambda: self._open_tab("New Tool", QLabel("Tool launched")))
        else:
            menu.addAction("🔍 View Details", lambda: QMessageBox.information(self, "Mock", "Viewing item"))
            menu.addAction("❌ Delete", lambda: QMessageBox.warning(self, "Mock", "Deleted item"))
