...
# NENHUM CABEÇALHO HARDCODED. Eles serão lidos dinamicamente da planilha
# ou definidos pelo usuário ao adicionar a primeira linha.

class ManufacturingTool(QWidget):
    """
    GUI para gerenciar dados de Fabricação (ordens de produção, processos, etc.).
    Permite visualizar, adicionar, editar e salvar informações de fabricação em planilhas Excel.
    ...
    def _add_empty_row(self):
        ...
        if self.table.columnCount() == 0 and row_count == 0:
            text, ok = QInputDialog.getText(self, "Definir Cabeçalhos", 
                                            "A planilha está vazia. Insira os nomes das colunas separados por vírgula (ex: ID da Ordem, Produto, Quantidade, Status):")
            if ok and text:
...
if __name__ == "__main__":
    ...
    # Cria/Atualiza um arquivo manufacturing_data.xlsx de teste com a sheet padrão 'Manufacturing'
    ...
    # Cria um novo workbook e salva-o com a sheet 'Manufacturing'
    wb_manuf = openpyxl.Workbook()
    ws_manuf = wb_manuf.active # Ativa a primeira sheet
    ws_manuf.title = DEFAULT_SHEET_NAME # Define o título como "Manufacturing"
    # Adiciona cabeçalhos e alguns dados de exemplo (ou deixar vazio para testar a criação de headers)
    ws_manuf.append(["ID da Ordem", "Produto", "Quantidade", "Status", "Data de Início", "Data de Término"])
    ws_manuf.append(["ORD-001", "Produto A", 100, "Em Andamento", "2024-06-01", "2024-06-15"])
    ws_manuf.append(["ORD-002", "Produto B", 50, "Concluído", "2024-05-20", "2024-06-10"])
    
    wb_manuf.save(test_file_path)
