...
DATA_EXCEL_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'user_sheets', 'RPI.xlsx')
SHEET_NAME = "manufacturing_orders" # Name of the sheet within RPI.xlsx for manufacturing data

class ManufacturingTool(QWidget):
    """
    GUI for managing Manufacturing Orders.
    Allows viewing, adding, and saving manufacturing information to 'RPI.xlsx'.
    """
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Ordens de Manufatura")
    ...
    def _load_data(self):
        ...
        try:
            if not os.path.exists(DATA_EXCEL_PATH):
                QMessageBox.warning(self, "Arquivo Não Encontrado", f"O arquivo de manufatura não foi encontrado: {DATA_EXCEL_PATH}. Criando um novo.")
            ...
            QMessageBox.critical(self, "Erro de Carregamento", f"Erro ao carregar dados de manufatura: {e}")
    ...
    def _save_data(self):
        ...
        try:
            ...
            if not current_headers:
                current_headers = ["ID Ordem", "Produto", "Quantidade", "Status", "Data Início", "Data Fim"]
            ...
            QMessageBox.critical(self, "Erro ao Salvar", f"Erro ao salvar dados de manufatura: {e}")
    ...
    def _create_new_excel_file(self):
        ...
        ws = wb.active
        ws.title = SHEET_NAME
        headers = ["ID Ordem", "Produto", "Quantidade", "Status", "Data Início", "Data Fim"]
        ws.append(headers)
        wb.save(DATA_EXCEL_PATH)
        self.table.setColumnCount(len(headers))
        self.table.setHorizontalHeaderLabels(headers)
