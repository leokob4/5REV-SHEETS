...
SHEET_NAME = "items_general" # Name of the sheet within output.xlsx for general items

class ItemsTool(QWidget):
    """
    GUI for managing Generic Items.
    Allows viewing, adding, and saving item information to 'output.xlsx'.
    """
    def __init__(self):
        super().__init__()
    ...
    def _load_data(self):
        ...
        try:
            if not os.path.exists(DATA_EXCEL_PATH):
                QMessageBox.warning(self, "Arquivo Não Encontrado", f"O arquivo de itens gerais não foi encontrado: {DATA_EXCEL_PATH}. Criando um novo.")
            ...
            QMessageBox.critical(self, "Erro de Carregamento", f"Erro ao carregar dados de itens gerais: {e}")
    ...
    def _save_data(self):
        ...
        try:
            ...
            if not current_headers:
                current_headers = ["ID Item", "Nome Item", "Tipo", "Status", "Localização"]
            ...
            QMessageBox.critical(self, "Erro ao Salvar", f"Erro ao salvar dados de itens gerais: {e}")
    ...
    def _create_new_excel_file(self):
        ...
        ws = wb.active
        ws.title = SHEET_NAME
        headers = ["ID Item", "Nome Item", "Tipo", "Status", "Localização"]
        ws.append(headers)
        wb.save(DATA_EXCEL_PATH)
        self.table.setColumnCount(len(headers))
        self.table.setHorizontalHeaderLabels(headers)
