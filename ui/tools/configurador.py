...
DATA_EXCEL_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'user_sheets', 'configurador.xlsx')
SHEET_NAME = "configurations" # Name of the sheet within configurador.xlsx

class ConfiguradorTool(QWidget):
    """
    GUI for managing Product Configurations.
    Allows viewing, adding, and saving configuration data to 'configurador.xlsx'.
    """
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Configurador de Produto")
    ...
    def _load_data(self):
        ...
        try:
            if not os.path.exists(DATA_EXCEL_PATH):
                QMessageBox.warning(self, "Arquivo Não Encontrado", f"O arquivo de configurações não foi encontrado: {DATA_EXCEL_PATH}. Criando um novo.")
            ...
            QMessageBox.critical(self, "Erro de Carregamento", f"Erro ao carregar dados de configuração: {e}")
    ...
    def _save_data(self):
        ...
        try:
            ...
            if not current_headers:
                current_headers = ["ID Config", "Nome Config", "Opção 1", "Opção 2", "Item Associado"]
    ...
    def _create_new_excel_file(self):
        ...
        ws = wb.active
        ws.title = SHEET_NAME
        headers = ["ID Config", "Nome Config", "Opção 1", "Opção 2", "Item Associado"]
        ws.append(headers)
        wb.save(DATA_EXCEL_PATH)
        self.table.setColumnCount(len(headers))
        self.table.setHorizontalHeaderLabels(headers)
