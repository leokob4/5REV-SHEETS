...
DATA_EXCEL_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'user_sheets', 'manutencao.xlsx')
SHEET_NAME = "maintenance_log" # Name of the sheet within manutencao.xlsx

class ManutencaoTool(QWidget):
    """
    GUI for managing Equipment Maintenance.
    Allows viewing, adding, and saving maintenance logs to 'manutencao.xlsx'.
    """
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Gerenciador de Manutenção")
    ...
    def _load_data(self):
        ...
        try:
            if not os.path.exists(DATA_EXCEL_PATH):
                QMessageBox.warning(self, "Arquivo Não Encontrado", f"O arquivo de manutenção não foi encontrado: {DATA_EXCEL_PATH}. Criando um novo.")
            ...
            QMessageBox.critical(self, "Erro de Carregamento", f"Erro ao carregar dados de manutenção: {e}")
    ...
    def _save_data(self):
        ...
        try:
            ...
            if not current_headers:
                current_headers = ["ID Manutenção", "Equipamento", "Tipo Manutenção", "Data", "Técnico", "Status"]
    ...
    def _create_new_excel_file(self):
        ...
        ws = wb.active
        ws.title = SHEET_NAME
        headers = ["ID Manutenção", "Equipamento", "Tipo Manutenção", "Data", "Técnico", "Status"]
        ws.append(headers)
        wb.save(DATA_EXCEL_PATH)
        self.table.setColumnCount(len(headers))
        self.table.setHorizontalHeaderLabels(headers)
