...
DATA_EXCEL_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'user_sheets', 'pedidos.xlsx')

class PedidosTool(QWidget):
    """
    GUI for managing Orders (Customer/Supplier).
    Allows viewing, adding, and saving order data to 'pedidos.xlsx'.
    """
    def __init__(self):
        super().__init__()
        ...
        self.sheet_name_input = QLineEdit("orders") # Default sheet name
    ...
    def _load_data(self):
        ...
        try:
            if not os.path.exists(DATA_EXCEL_PATH):
                QMessageBox.warning(self, "Arquivo Não Encontrado", f"O arquivo de pedidos não foi encontrado: {DATA_EXCEL_PATH}. Criando um novo.")
            ...
            QMessageBox.critical(self, "Erro de Carregamento", f"Erro ao carregar dados de pedidos: {e}")
    ...
    def _save_data(self):
        ...
        try:
            ...
            if not current_headers:
                current_headers = ["ID Pedido", "Tipo", "Cliente/Fornecedor", "Data Pedido", "Status", "Valor Total"]
    ...
    def _create_new_excel_file(self, sheet_name):
        ...
        ws = wb.active
        ws.title = sheet_name
        headers = ["ID Pedido", "Tipo", "Cliente/Fornecedor", "Data Pedido", "Status", "Valor Total"]
        ws.append(headers)
        wb.save(DATA_EXCEL_PATH)
        self.table.setColumnCount(len(headers))
        self.table.setHorizontalHeaderLabels(headers)
