...
SHEET_NAME = "bom_data" # Name of the sheet within RPI.xlsx for BOM data

class BomManagerTool(QWidget):
    """
    GUI for managing Bill of Materials (BOMs).
    Allows viewing, adding, and saving BOM information to 'RPI.xlsx'.
    """
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Gerenciador de BOM")
    ...
    def _load_data(self):
        ...
        try:
            if not os.path.exists(DATA_EXCEL_PATH):
                QMessageBox.warning(self, "Arquivo Não Encontrado", f"O arquivo de dados BOM não foi encontrado: {DATA_EXCEL_PATH}. Criando um novo.")
            ...
            QMessageBox.critical(self, "Erro de Carregamento", f"Erro ao carregar dados BOM: {e}")
    ...
    def _save_data(self):
        ...
        try:
            ...
            if not current_headers:
                current_headers = ["ID da BOM", "Item Pai", "Item Componente", "Quantidade", "Unidade"]
            ...
            QMessageBox.critical(self, "Erro ao Salvar", f"Erro ao salvar dados BOM: {e}")
    ...
    def _create_new_excel_file(self):
        ...
        ws = wb.active
        ws.title = SHEET_NAME
        headers = ["ID da BOM", "Item Pai", "Item Componente", "Quantidade", "Unidade"]
        ws.append(headers)
        wb.save(DATA_EXCEL_PATH)
        self.table.setColumnCount(len(headers))
        self.table.setHorizontalHeaderLabels(headers)
