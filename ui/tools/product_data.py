...
PRODUCT_HEADERS = ["ID", "Nome do Produto", "Código", "Revisão", "Descrição"]

class ProductDataTool(QWidget):
    """
    GUI para gerenciar Dados do Produto.
    Permite visualizar, adicionar e salvar informações do produto.
    ...
    def _populate_sheet_selector(self):
        ...
        if not os.path.exists(self.file_path):
            ...
            self.table.setColumnCount(len(PRODUCT_HEADERS))
        ...
        try:
            ...
            if not os.path.exists(self.file_path):
                ...
                if not headers_to_save:
                    headers_to_save = PRODUCT_HEADERS
            ...
            if not current_headers:
                current_headers = PRODUCT_HEADERS
