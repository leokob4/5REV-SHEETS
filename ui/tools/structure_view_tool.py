...
class StructureViewTool(QWidget):
    ...
    """
    def __init__(self, file_path, sheet_name=None): # Default sheet for engenharia should be handled inside populate
        super().__init__()
        self.file_path = file_path
        self.sheet_name = sheet_name # This will be set by populate_sheet_selector
    ...
    def _populate_sheet_selector(self):
        ...
        if not os.path.exists(self.file_path):
            QMessageBox.warning(self, "Arquivo Não Encontrado", f"O arquivo de dados não foi encontrado: {os.path.basename(self.file_path)}.")
            return
    ...
    def _load_structure_data(self):
        ...
        try:
            ...
            if not root_items_ids:
                QMessageBox.information(self, "Nenhuma Estrutura Encontrada", f"Nenhum dado de estrutura hierárquica válido encontrado na planilha '{current_sheet_name}' do arquivo '{os.path.basename(self.file_path)}'. Verifique se 'parent_part_number/ParentID' e 'part_number/ComponentID' estão presentes e corretos.")
            ...
            for root_id in root_items_ids:
                ...
                if root_data:
                    ...
                    dummy_root_item = QTreeWidgetItem([root_id] + [""] * (len(headers) - 1)) 
...
if __name__ == "__main__":
    ...
    if not os.path.exists(test_bom_file_path):
        ...
        ws1 = wb_test.active
        ws1.title = "BOM"
        ws1.append(["ID do BOM", "ID do Componente", "Nome do Componente", "Quantidade", "Unidade", "Ref Designator"])
