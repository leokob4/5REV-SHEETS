...
DATA_EXCEL_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'user_sheets', 'financeiro.xlsx')
SHEET_NAME = "transactions" # Name of the sheet within financeiro.xlsx

class FinanceiroTool(QWidget):
    """
    GUI for managing Financial data.
    Allows viewing, adding, and saving financial transactions to 'financeiro.xlsx'.
    """
    def __init__(self):
        super().__init__()
    ...
    def _load_data(self):
        ...
        try:
            if not os.path.exists(DATA_EXCEL_PATH):
                QMessageBox.warning(self, "Arquivo Não Encontrado", f"O arquivo financeiro não foi encontrado: {DATA_EXCEL_PATH}. Criando um novo.")
            ...
            QMessageBox.critical(self, "Erro de Carregamento", f"Erro ao carregar dados financeiros: {e}")
    ...
    def _save_data(self):
        ...
        try:
            ...
            if not current_headers:
                current_headers = ["ID Transação", "Tipo", "Valor", "Data", "Descrição", "Conta"]
    ...
    def _create_new_excel_file(self):
        ...
        ws = wb.active
        ws.title = SHEET_NAME
        headers = ["ID Transação", "Tipo", "Valor", "Data", "Descrição", "Conta"]
        ws.append(headers)
        wb.save(DATA_EXCEL_PATH)
        self.table.setColumnCount(len(headers))
        self.table.setHorizontalHeaderLabels(headers)
