...
DEFAULT_DATA_EXCEL_FILENAME = "programacao.xlsx"
DEFAULT_SHEET_NAME = "Programacao" # Alterado para "Programacao" para ser mais descritivo

# PCP_HEADERS FOI REMOVIDO. Os cabeçalhos serão lidos dinamicamente da planilha
...
class PcpTool(QWidget):
    ...
    Permite visualizar, adicionar, editar e salvar informações de PCP em planilhas Excel.
    ...
    def __init__(self, file_path=None):
        ...
        self.setWindowTitle(f"PCP: {os.path.basename(self.file_path)}")
    ...
    def _load_data_from_selected_sheet(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Erro de Carregamento", f"Erro ao carregar dados de PCP da aba '{current_sheet_name}': {e}")
            self.table.setRowCount(0)
            self.table.setColumnCount(0) 
    ...
    def _save_data(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Erro ao Salvar", f"Erro ao salvar dados de PCP: {e}")
    ...
    def _add_empty_row(self):
        ...
        if self.table.columnCount() == 0 and row_count == 0:
            text, ok = QInputDialog.getText(self, "Definir Cabeçalhos", 
                                            "A planilha está vazia. Insira os nomes das colunas separados por vírgula (ex: ID da Ordem, Produto, Data de Início, Status):")
            if ok and text:
...
if __name__ == "__main__":
    ...
    # Cria/Atualiza um arquivo programacao.xlsx de teste com a sheet padrão 'Programacao'
    ...
    # Cria um novo workbook e salva-o com a sheet 'Programacao'
    wb_pcp = openpyxl.Workbook()
    ws_pcp = wb_pcp.active # Ativa a primeira sheet
    ws_pcp.title = DEFAULT_SHEET_NAME # Define o título como "Programacao"
    # Adiciona cabeçalhos e alguns dados de exemplo (ou deixar vazio para testar a criação de headers)
    ws_pcp.append(["ID da Ordem", "Produto", "Data de Início", "Data de Término", "Prioridade", "Status"])
    ws_pcp.append(["PCP-001", "Produto A", "2024-06-01", "2024-06-15", "Alta", "Planejado"])
    ws_pcp.append(["PCP-002", "Produto C", "2024-06-10", "2024-06-20", "Média", "Em Produção"])
    
    wb_pcp.save(test_file_path)
    print(f"Arquivo de teste '{DEFAULT_DATA_EXCEL_FILENAME}' criado/atualizado com abas de exemplo.")

    window = PcpTool(file_path=test_file_path)
    window.show()
    sys.exit(app.exec_())
