...
DATA_EXCEL_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'user_sheets', 'programacao.xlsx')
SHEET_NAME = "production_plan" # Name of the sheet within programacao.xlsx for PCP data

class PcpTool(QWidget):
    """
    GUI for managing Production Planning and Control (PCP).
    Allows viewing, adding, and saving PCP data to 'programacao.xlsx'.
    """
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Planejamento e Controle da Produção (PCP)")
    ...
    def _load_data(self):
        ...
        try:
            if not os.path.exists(DATA_EXCEL_PATH):
                QMessageBox.warning(self, "Arquivo Não Encontrado", f"O arquivo de PCP não foi encontrado: {DATA_EXCEL_PATH}. Criando um novo.")
            ...
            QMessageBox.critical(self, "Erro de Carregamento", f"Erro ao carregar dados de PCP: {e}")
    ...
    def _save_data(self):
        ...
        try:
            ...
            if not current_headers:
                current_headers = ["ID Plano", "Produto", "Quantidade", "Data Início", "Data Fim", "Status"]
    ...
    def _create_new_excel_file(self):
        ...
        ws = wb.active
        ws.title = SHEET_NAME
        headers = ["ID Plano", "Produto", "Quantidade", "Data Início", "Data Fim", "Status"]
        ws.append(headers)
        wb.save(DATA_EXCEL_PATH)
        self.table.setColumnCount(len(headers))
        self.table.setHorizontalHeaderLabels(headers)
